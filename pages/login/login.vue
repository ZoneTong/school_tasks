	...
	export default {
		...
		methods: {
			...
			getUserInfo({
				...
				if (detail.userInfo) {
					this.toMain(detail.userInfo.nickName, detail.userInfo.avatarUrl);
				} else {
					uni.showToast({
						icon: 'none',
						title: '登陆失败'
			...
			toMain(userName, avatarUrl) {
				...
				if (this.forcedLogin) {
					...
					console.log("haslogin", this.$store.state.hasLogin, this.$store.state.userName, this.$store.state.avatarUrl)
					uni.navigateBack();
				}
