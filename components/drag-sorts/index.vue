			...
			<movable-view v-for="(item, index) in currentList" :key="index" :x="item.x" v-if="item.isShow === 1" :data-index="index"
			 ...
			 :class="{'active': active == index, 'vh-1px-t': item.index > 0}">
				<view class="itembox" :style="{color: styleObject.color, background: styleObject.background}">
					<text class="itemtext">{{item[props.label]}}</text>
					<view v-if="closable"  @click="close(index)">
						<faicon type="close" color="red" size="14"></faicon>
					</view>
				</view>
			</movable-view>
		</movable-area>
		...
		methods: {
			...
			moveUpdateCurrentList(index) {
				...
				if (index < 0) {
					...
					// 	if ( this.currentList[i].isShow > 0){
					// 		orders.push(this.currentList[i].SortNumber)
					// 	}						
					// }
					// console.log('zht orders:', orders);
				}
			},
			touchstart(e) {
				// 计算 x y 轴点击位置
						...
						this.currentList.every((_res, _index) => {
							...
							if (this.currentList[this.active].SortNumber < this.currentList[index].SortNumber) {
								...
								if (this.currentList[_index].SortNumber > this.currentList[this.active].SortNumber && this.currentList[
									...
									_res.SortNumber--
									// console.log('zht--', _res, _index,this.active, index, absX, absY,'topX', this.topX, this.topY, this.windowWidth );
								}
							} else {
								// 反之++
								if (this.currentList[_index].SortNumber < this.currentList[this.active].SortNumber && this.currentList[
			...
			close(index) {
				...
				console.log("我是删除的index 的SortNumber", index, this.currentList[index].SortNumber)		
